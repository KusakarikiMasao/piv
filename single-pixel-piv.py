...
def correlation(x, y, cal_point, img0, img1, param_xsearch, param_ysearch):
    ...
    vector_u = np.zeros(x.shape)
    vector_v = np.zeros(x.shape)
    print(img0.shape)
    print(img1.shape)
        ...
        for j in range(x.shape[1]):
            if cal_point[i, j]:
                ...
                try:
                    ...
                    corr_map = np.zeros((param_ysearch[1]-param_ysearch[0]+1, param_xsearch[1]-param_xsearch[0]+1))
                    for i1 in range(param_xsearch[0], param_xsearch[1]+1):
                        for j1 in range(param_ysearch[0], param_ysearch[1]+1):#search windowでの繰り返し
                            if (0 <= x_coord+i1 < img1.shape[1] and 0 <= y_coord+j1 < img1.shape[0]):
                                data1 = img1[int(y_coord+j1), int(x_coord+i1), :]
                                corr_map[j1-param_ysearch[0], i1-param_xsearch[0]] = np.corrcoef(data0, data1)[0, 1]#相互相関係数の計算
